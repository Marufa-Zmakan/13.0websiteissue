...
class picking_order(models.Model):
    ...
    def action_picking_auto_notification(self):
        if self['state'] in ['assigned']:
            model_id = self.env['ir.model'].sudo().search([('model', '=', 'picking.order')]).id
            activity_type = self.env['ir.config_parameter'].sudo().get_param(
                'pragmatic_odoo_delivery_boy.zts_notification_dri')
    ...
    def create(self, vals):
        ...
        self.update_picking_order_payment_status(vals)
    ...
    def update_picking_order_payment_status(self,vals):
        order=self.env['sale.order'].sudo().browse(vals['sale_order'])
        last_tx = order.transaction_ids.get_last_transaction()
        if last_tx and last_tx.acquirer_id.journal_id.type != 'cash':
            vals['payment'] = 'paid'
            order.delivery_state = 'paid'
